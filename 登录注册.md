...
- 后端前缀 `/api`，响应包络为 `{ code, msg, ...平铺字段 }`（注意是 `msg` 不是 `message`，且无 `data` 嵌套），与当前前端 `useApi` 假定的 `{ code, message, data }` **不一致**，鉴权接口需单独用 `$fetch` 处理。
...
- 新增 `app/apis/auth.ts`：用 `$fetch`（baseURL 取 runtimeConfig，注入 Bearer 头）实现 `registerApi / loginApi / refreshApi / logoutApi / fetchMeApi`，返回类型精确匹配后端字段（`msg`、平铺 token 字段、`user`）。
...
- 统一包络：成功 HTTP 200 + `code:200` + 字段平铺；失败 HTTP 状态码 == body.code + `{ code, msg }`。
