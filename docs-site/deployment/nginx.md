...
1. 反向代理到 Nuxt upstream `nuxt:3000`（含 WebSocket `Upgrade` 透传，供 dev HMR 使用）
2. 对 `/_nuxt/` 静态资源设置长缓存
...
| `/`、`/news/*` 等页面 | 代理到 Nuxt                                                               |
| `/_nuxt/*`            | 代理 + 长缓存                                                             |
| `/api/revalidate`     | 代理到 Nuxt（`location /` 兜底，网关没有 `/api` 排除规则）                |
| 后端业务 API          | **不经过本网关** —— 浏览器直连 `NUXT_PUBLIC_API_BASE` 指向的另一个 origin |

后端业务 API 之所以不过网关，是因为它在另一个 origin，而不是配置里做了路径排除；
