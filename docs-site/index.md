...
hero:
  ...
  text: 完整架构文档
  tagline: 通用 SaaS 前端基座 — 从零理解公开官网、SEO、多语言、账号、工作台、项目、编辑器、请求、主题与部署
  actions:
    - theme: brand
      text: 快速开始
...
features:
  ...
  - icon: 🔐
    title: 可选鉴权
    details: Bearer Token + Cookie + Pinia，命名 auth 中间件保护登录后产品路由，保留轻量权限扩展位。
  - icon: 🚀
    title: 部署就绪
    details: Nitro node-server、Docker、Compose、Nginx 样例与完整环境变量说明。
  - icon: 📝
    title: 工作台与编辑器
    details: 项目列表/删除、/docs/new 草稿创建、YanivEditor 编辑器（`EDITOR_AUTOSAVE_DEBOUNCE_MS` 2s 自动保存）、账户中心与可选主题模板占位。
---
