...
**全部纳入项已完成,`pnpm quality` 与 `docs:sync:check` 全绿,测试 126 → 137。**

| #   | 状态        | 反向验证(故意引入缺陷 → 检查必须失败)                                      |
| --- | ----------- | -------------------------------------------------------------------------- |
| 1   | ✅          | 给 `account.vue` 加别名深引 → `page-structure` **失败** ✓                  |
| 2   | ✅          | `slice(0,16)`→`(0,15)` → **失败** ✓;group 改 `nitro/handlers` → **失败** ✓ |
| 3   | ✅          | `timingSafeEqual(sha256)` / 限流 / 白名单,三条各有单测                     |
| 4   | ✅          | 13 条对抗输入行为实测 **13/13** ✓(含 `\t` `\n` `\r` 空格 `%2F` `%5C`)      |
| 5   | ✅          | `/docs/1?ts=10:30` 放行、`/javascript:alert(1)` 拒绝 ✓                     |
| 6   | ✅          | 停用 antdv 分包分支 + 重新 build → **失败** ✓                              |
| 7   | ✅          | 给 `features/templates` 加跨 feature 引用 → **失败** ✓                     |
| 8   | ✅          | 选 **B**:`nuxt.config.ts:126-128` 写明约束(记录约束 ≠ 留兼容层)            |
| 9   | ❌ **不做** | 用户 2026-07-31 明确决定不改 `.env.*` 提交方式                             |
| 10  | ✅          | 约定已记录                                                                 |
| 11  | —           | 不纳入(覆盖缺口,非缺陷)                                                    |

### 第二轮补完的两项(首轮只做了便宜的一半)

首轮 #2 / #6 都**部分复现了 §5 的贯穿性根因**(测试锁在错误的层),已补完:

|        | 首轮做到的                          | 首轮漏掉的                                                                                             | 本轮补上                                                                                                               |
| ------ | ----------------------------------- | ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| **#2** | 静默 200 → 500/207,运行时失败变响亮 | 锁 Nitro 真实 key 的测试。旧的「前缀 + 幂等」断言**原样保留**,且用被测函数自己播种假 storage(循环论证) | 新增 `tests/unit/revalidate-nitro-contract.test.ts`,**从 nitropack 源码提取真实算法与常量执行比对**;删除被取代的旧断言 |
| **#6** | 从读源码文本改成读 `.output`        | 只断言体积,**不断言 4 个 vendor chunk 存在** —— 分包退化只要没超 3MB 照样绿                            | 从 `client.precomputed.mjs` 读 chunk 名映射,断言 4 个 vendor chunk 真实产出;`return` 改 `ctx.skip()`                   |

#### #2 的实现要点(为什么不用起服务器)

实测 nitropack 2.13.4 的契约链:

```
cache.mjs:124  escapeKey(key) = String(key).replace(/\W/g,'')
cache.mjs:139  pathname = escapeKey(decodeURI(parseURL(path).pathname)).slice(0, 16) || 'index'
cache.mjs:143  hashedPath = `${pathname}.${hash(path)}`
cache.mjs:13   defaultCacheOptions = { name: '_', base: '/cache', swr: true, maxAge: 1 }
app.mjs:131    routeRules.cache → group: 'nitro/routes'
cache.mjs      cacheKey = [base, group, name, key+'.json'].filter(Boolean).join(':')
```

关键发现:`base` 是 **`/cache`(带前导斜杠)**,而项目用 `useStorage('cache')`。
两者靠 **unstorage 的 `/` → `:` 归一化**才等价 —— 这是一条极隐蔽的依赖,
正是本测试要锁住的东西。

因为 SWR cache 在生产构建下是**内存驱动**(无 `.data` / `.nitro` 目录),
起 `nuxt preview` 子进程读不到它的内存。所以测试改为:
用 Nitro 的算法往 `unstorage` 内存实例写入条目 → 用项目的 key 经 `prefixStorage(storage,'cache')` 查找并清除
→ **无需服务器即可端到端证明「我们的 key + 我们的存储访问 == Nitro 的 key + Nitro 的存储访问」**。

#### 一处踩坑记录

首次反向验证 #6 时我把破坏写成 `false && id.includes('ant-design-vue')`,
但原分支是 `if (A || B)`,`B`(`@ant-design/icons-vue`)仍然成立 → chunk 照常产出 → 测试假绿。
**反向验证本身也会有 bug** —— 必须确认「破坏确实生效」(检查产物里 chunk 名真的消失了)再判定。
...
| ~~9~~ | ~~`.env.prod` 被 gitignore 白名单强制提交~~   | —     | ❌ **用户决定不做**(2026-07-31) |
