...
- 新增 `app/apis/auth.ts`：用 `$fetch`（baseURL 取 runtimeConfig，注入 Bearer 头）实现 `registerApi / loginApi / refreshApi / logoutApi / fetchMeApi`，只在 API adapter 边界感知后端 `msg` 与平铺字段；store 和页面消费归一化后的 `message`、token 字段、`user`。受保护 Auth API 遇到 401 时复用单飞 refresh 并重试一次。
