...
- 后端前缀 `/api`，鉴权响应包络为 `{ code, msg, ...平铺字段 }`（注意是 `msg` 不是 `message`，且无 `data` 嵌套）。前端把这种后端差异限制在 `app/apis/auth/index.ts`：鉴权接口单独用 `$fetch` 请求，并通过 `normalizeFlatApiResponse()` 统一归一化为应用内 `message` 字段。
...
flowchart TD
  ...
  page --> apiCall["useApi/Auth API 受保护请求自动带 Bearer 头"]
...
- 新增 `app/api-core` 与 `app/utils/api-contract.ts`：定义应用内 `{ code, message, data }` 契约和 `normalizeFlatApiResponse()`，用于把后端平铺响应的 `msg` 归一化为 `message`。
- 新增 `app/apis/auth/index.ts`：用 `$fetch`（baseURL 取 runtimeConfig，注入 Bearer 头）实现 `registerApi / loginApi / refreshApi / logoutApi / fetchMeApi`，只在 API adapter 边界感知后端 `msg` 与平铺字段；store 和页面消费归一化后的 `message`、token 字段、`user`。受保护 Auth API 遇到 401 时复用单飞 refresh 并重试一次。
- 修改 `app/composables/useApi.ts`：从 token cookie（`useCookie`，server + client 通用）自动注入 `Authorization: Bearer`；401 触发一次 `refresh` 重试的单飞逻辑（失败则清会话）。保持现有 header 转发与脱敏。
...
- 修改 `nuxt.config.ts` / `.env.dev` 等：只保留 `NUXT_PUBLIC_API_BASE`，前端 SSR 与浏览器请求都直连后端 API；跨域由后端或网关 CORS 策略处理。
