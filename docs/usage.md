...
- `POST /api/v1/register` creates an account but does not log the user in. The sign-up page redirects users to sign-in after success.
- `POST /api/v1/login` and `POST /api/v1/refresh` return `accessToken` and `refreshToken`. Tokens are stored in JS-readable Nuxt cookies for client-side product workflows.
- `createAuthApiClient()` and `createProductApiClient()` may attach the access token for authenticated business requests. A 401 triggers a single-flight `POST /api/v1/refresh` and retries the failed request once; refresh failure clears the local session.
...
- `app/plugins/auth.client.ts` hydrates `/api/v1/me` on startup when token cookies are present. It is client-only on purpose: prerendered and SWR-cached HTML must not depend on who is signed in.
...
- The backend has account roles (`Users.role`: `user` / `admin`) but deliberately keeps them out of the JWT and `/api/v1/me`: shipping roles to the browser invites the frontend to treat them as authorization, while the real check happens server-side per request. `normalizeAuthUser()` therefore leaves `roles` and `permissions` empty, so `route.meta.auth.roles` never matches and such a page would 403 for everyone. This is the agreed split, not a gap waiting to be filled — if the UI ever needs to hide admin-only affordances, request a purpose-built capability signal and treat it as presentation only.
...
- Auth cookies use `secure: true` when `runtimeConfig.public.appEnv === 'production'` **or** `runtimeConfig.public.siteUrl` starts with `https://`, so an HTTPS staging environment does not silently drop the flag. Docker Compose sets `NUXT_PUBLIC_APP_ENV=production` for the production stack.
