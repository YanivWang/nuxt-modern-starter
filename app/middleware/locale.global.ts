...
const isLocaleLikePrefix = (segment?: string) =>
  Boolean(segment && /^[a-z]{2}(-[a-z]{2})?$/i.test(segment))
 ...
 */
export const resolveLocaleRouteDecision = (
  path: string,
  ...
  const resolvedLocale = resolvePreferredLocale(path, persistedLocale) as SupportedLocale
...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  const persistedLocale = resolveStoredLocale(languageCookie.value)
  const decision = resolveLocaleRouteDecision(to.path, persistedLocale)
