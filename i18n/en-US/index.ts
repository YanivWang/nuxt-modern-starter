/*
  【文件职责】
    英文 UI 文案包：nav、brand、auth、home、editor、workspace、about、help、news、error 等域。
    与 i18n/zh-CN/index.ts 结构镜像，由 i18n/index.ts 异步按需加载。

  【架构位置】
    i18n 层 — 非默认 locale 文案，动态 import 以减小首屏 bundle。
  ...
  【依赖关系】
    - 依赖：无
    - 被引用：i18n/index.ts（LOCALE_LANGUAGE_MODULES['en-US']）

  【渲染 / 数据】
    无 — 纯静态文案；禁止逐 key 行内注释。

  【边界与注意】
    新增文案域时须同步 zh-CN/index.ts；key 命名与页面 / feature 模块对齐。
*/
export default {
  nav: {
    home: 'Home',
